...
<div x-data="{ toggleCreate: false }">
    <div>
        <div class="flow-root ">
            <h1 class="float-left">{{ __('Categories') }}</h1>
            <x-button wire:click="resetCategory" @click="toggleCreate = !toggleCreate" class="float-right">
                {{ __('Create Category') }}</x-button>
        </div>

        <div x-cloak x-show="toggleCreate" x-transition.otigin.top.right class="card">
            <h3 class="mb-4"> {{ $editing ? __('Update') : __('Create') }} {{ __(' Category') }}</h3>
            <x-form wire:submit.prevent="{{ $editing ? 'update' : 'create' }}" method="POST">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-form.input wire:model="category.name" name="category" :label="__('Category Name')" />
                </div>
                <div>

                    <x-button>{{ $editing ? __('Update') : __('Create') }}</x-button>
                    <x-button type="button" wire:click="resetCategory" class="ml-2 absolute">
                ...
                @foreach ($categories as $category)
                    <tr>
                        ...
                        <td class="flex space-x-2">
                            @can('edit_users')
                                <a href="#" wire:click="edit({{ $category->id }})" @click="toggleCreate = true">
                                    {{ __('Edit') }}</a>
                                {{-- <a href="{{ route('admin.users.show', $category) }}">{{ __('Profile') }}</a> --}}
                            @endcan
                            @if (can('delete_users'))
                                <x-modal>
                                    <x-slot name="trigger">
                                    ...
                                    <x-slot name="content">
                                        <div class="text-center">
                                            {{ __('Are you sure you want to delete') }}: <b>{{ $category->name }}</b>
                                        </div>
                                    </x-slot>
                                    ...
                                    <x-slot name="footer">
                                        ...
                                        <button class="btn btn-red"
                                            wire:click="delete('{{ $category->id }}')">{{ __('Delete Category') }}</button>
                                    </x-slot>
                                </x-modal>
                            @endif
                        </td>
